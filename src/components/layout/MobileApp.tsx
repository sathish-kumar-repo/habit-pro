    ...
    <div className="flex h-screen w-full flex-col overflow-hidden bg-background lg:hidden">
      ...
      >
        <div className="mx-auto w-full max-w-2xl px-4 sm:px-6">
          ...
          {tab === "today" && (
            ...
            <div className="space-y-4 pb-6 pt-4">
              ...
              <div className="rounded-2xl border border-border bg-card p-4" style={{ boxShadow: "var(--shadow-soft)" }}>
                <DateStrip habits={p.habits} selectedDate={p.selectedDate} setSelectedDate={p.setSelectedDate} />
                <div className="mt-3 border-t border-[oklch(1_0_0_/_0.06)] pt-3">
                  <h1 className="font-display text-2xl leading-tight text-foreground">
                    {p.selectedDate > (() => { const d = new Date(); d.setHours(0, 0, 0, 0); return d; })()
                      ? p.statsForSelectedDate.active > 0
                        ? `${p.statsForSelectedDate.active} habit${p.statsForSelectedDate.active !== 1 ? "s" : ""} planned`
                        : "Nothing planned"
                      : p.statsForSelectedDate.done === p.statsForSelectedDate.active && p.statsForSelectedDate.active > 0
                        ? "All done! 🎉"
                        : p.statsForSelectedDate.active === 0
                          ? "No habits this day"
              ...
              <div className="flex items-center gap-5 rounded-2xl border border-border bg-card p-4" style={{ boxShadow: "var(--shadow-soft)" }}>
                <div className="relative shrink-0" aria-hidden>
                  <svg width="96" height="96" viewBox="0 0 96 96">
                    <circle cx="48" cy="48" r={r} fill="none" strokeWidth="6" stroke="oklch(1 0 0 / 0.06)" />
                    <circle
                      cx="48" cy="48" r={r} fill="none" strokeWidth="6"
                      stroke="var(--color-primary)"
                      strokeDasharray={circ}
                      strokeDashoffset={circ * (1 - (p.statsForSelectedDate.active ? p.statsForSelectedDate.done / p.statsForSelectedDate.active : 0))}
                      strokeLinecap="round" transform="rotate(-90 48 48)"
                      style={{ transition: "stroke-dashoffset 0.7s cubic-bezier(0.4,0,0.2,1)" }}
                    />
                  </svg>
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="font-display text-2xl leading-none text-foreground">{p.statsForSelectedDate.done}</span>
                    <span className="font-mono text-[10px] text-muted-foreground">/{p.statsForSelectedDate.active}</span>
                  </div>
                </div>
              ...
              <div>
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="font-display text-xl text-foreground">
                    {p.selectedDateKey === fmtDate(new Date()) ? "Today's habits" : `${p.selectedDate.toLocaleDateString("en", { weekday: "long" })}'s habits`}
                  </h2>
                ...
                <TodayHabitList
                  ...
                  emptySlot={
                    <div className="rounded-2xl border border-dashed border-border bg-card/40 px-6 py-10 text-center">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">Select today or a date with active habits.</p>
                    ...
                    return (
                      <div key={d.key} className="flex flex-1 flex-col items-center gap-1.5">
                        <div className="relative w-full" style={{ height: 64 }}>
                          <div className="absolute inset-x-1 bottom-0 rounded-sm" style={{ height: "100%", background: isSelDay ? "oklch(1 0 0 / 0.07)" : "oklch(1 0 0 / 0.04)" }} />
                          <div
                            className="absolute inset-x-1 bottom-0 rounded-sm transition-all"
                            style={{
                              height: hh,
                              background: isSelDay || isToday ? "linear-gradient(180deg,var(--color-primary),oklch(0.62 0.16 158))" : "linear-gradient(180deg,oklch(1 0 0/.2),oklch(1 0 0/.07))",
          ...
          {tab === "habits" && (
            <MobileHabits
              filter={filter} setFilter={setFilter} counts={counts} filtered={filtered}
              toggleToday={toggleToday} setOpenId={setOpenId} setEditId={setEditId}
              handleDelete={handleDelete} habitSearch={p.habitSearch} setHabitSearch={p.setHabitSearch}
            />
          )}
          ...
          {tab === "progress" && (
            <MobileProgress
              stats={p.stats} rollup={p.rollup} rollup30={p.rollup30} rollupMonthly={p.rollupMonthly}
              habits={p.habits} counts={p.counts} progressRange={p.progressRange} setProgressRange={p.setProgressRange}
              progressSort={p.progressSort} setProgressSort={p.setProgressSort} setOpenId={p.setOpenId}
              weekdayStats={p.weekdayStats} periodComparison={p.periodComparison}
              consistencyScore={p.consistencyScore} perHabitExtended={p.perHabitExtended}
          ...
          {BOTTOM_NAV.map((t) => {
            ...
            return (
              ...
              >
                ...
                {active && (
                  <span className="absolute bottom-0 left-1/2 h-0.5 w-8 -translate-x-1/2 rounded-full" style={{ background: "var(--color-primary)" }} />
                )}
              </button>
            );
          })}
